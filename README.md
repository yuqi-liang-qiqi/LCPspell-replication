...
The distance methods used in the paper (OM, OMspell, OMspellRS, LCP, LCPspell, and their reverse variants) are implemented and maintained in the Sequenzo Python package. The same package also ships several built-in example datasets used in the appendix index plots (for example, mvad). To run any script in this repository, install Sequenzo in your Python environment and import from it. All simulation and empirical code here calls Sequenzo's `get_distance_matrix()`, clustering utilities, and data loaders rather than compiling local C++ sources.
